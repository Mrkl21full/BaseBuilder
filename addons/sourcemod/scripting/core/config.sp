void SetupConfig()
{
	...
	g_cLoadConvars = AutoExecConfig_CreateConVar("bb_load_convars", "1", "Load default convars located in basebuilder config folder?", _, true, 0.0, true, 1.0);
	g_cWarmupTime = AutoExecConfig_CreateConVar("bb_warmup_time", "20", "Time for warmup. (0 to disable warmup)", _, true, 0.0);
	...
	g_cMoveBlocks = AutoExecConfig_CreateConVar("bb_move_blocks", "z", "Admin flags to allow player to move blocks despite of thier team");
	g_cLockBlocks = AutoExecConfig_CreateConVar("bb_lock_blocks", "z", "Admin flags to allow player to lock (and unlock) blocks despite of thier team");

	g_cBlockRotation = AutoExecConfig_CreateConVar("bb_block_rotation", "5;15;25;35;45;90", "Available degress to change (up to 6)");
